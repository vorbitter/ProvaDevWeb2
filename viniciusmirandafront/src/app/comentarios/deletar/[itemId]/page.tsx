...
export default function DeletarComentario() {
  ...
  useEffect(() => {
    api
      .get<Comentario[]>(`/comentario/listar/${itemId}`)
  ...
  const handleDelete = async () => {
    ...
    const confirmado = confirm(
      "Tem certeza que deseja deletar este comentario?"
    );
    if (!confirmado) return;

    try {
      setLoading(true);
      await api.delete(`/comentario/deletar/${itemId}`);
      alert("Comentario deletado com sucesso!");
      setComentarios(
        comentarios.filter((p) => p.comentarioId !== comentarioId)
      );
